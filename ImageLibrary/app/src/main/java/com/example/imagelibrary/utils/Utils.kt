...
// 리스트의 형태로 데이터 저장
fun saveBookmarkData(context: Context, key: String, values: List<LockerModel>) {
    val gson = Gson()
...
// 데이터 로드
fun loadBookmarkData(context: Context, key: String): List<LockerModel> {
    val prefs = context?.getSharedPreferences("SETTINGS", Context.MODE_PRIVATE)
    val json = prefs?.getString(key, null)

    return if (json != null) {
        val gson = Gson()
        val storedData: List<LockerModel> =
            gson.fromJson(json, object : TypeToken<List<LockerModel>>() {}.type)
        storedData
    } else {
